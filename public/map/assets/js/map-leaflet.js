$(document).ready(function($) {
        ...
        for (var i = 0; i < loadedMarkersData.length; i++) {
            ...
            markerContent.innerHTML =
                '<div class="ts-marker-wrapper">' +
                    ...
                    ( ( loadedMarkersData[i]["marker_image"] !== undefined ) ? '<div class="ts-marker__image ts-black-gradient" style="background-image: url(' + loadedMarkersData[i]["marker_image"] + ')"></div>' : '<div class="ts-marker__image ts-black-gradient" style="background-image: url(../map/assets/img/marker-default-img.png)"></div>' ) +
                    '</a>' +
                '</div>';
        ...
        for (i = 0; i < visibleMarkersOnMap.length; i++) {
            ...
            resultsHtml.push(
                '<div class="ts-result-link" data-ts-id="' + loadedMarkersData[id]["id"] + '" data-ts-ln="' + newMarkers[id].loopNumber + '">' +
                    '<span class="ts-center-marker"><img src="../map/assets/img/result-center.svg"></span>' +
    ...
    if( $("#"+simpleMapId).length ){
        ...
        var icon = L.icon({
            iconUrl: "../map/assets/img/marker-small.png",
            iconSize: [22, 29],
            iconAnchor: [11, 29]
        });
