...
var handleBarChart = function (d1,label) {
	'use strict';
	if ($('#bar-chart').length !== 0) {
        var data = d1;
        var ticks = [[0, 'JAN'], [1, 'FEB'], [2, 'MAR'], [3, 'APR'], [4, 'MAY'], [5, 'JUN'],[6, 'JUL'],[7,'AUG'],[8,'SEP'],[9,'OCT'],[10,'NOV'],[11, 'DEC']];
        $.plot('#bar-chart', [{ label: label, data: data, color: COLOR_BLACK_LIGHTER }], {
            series: {
                bars: {
                    show: true,
                    barWidth: 0.8,
...
var handleInteractiveChart = function (d1, label) {
	'use strict';
	...
	if ($('#interactive-chart').length !== 0) {
        ...
        $.plot($('#interactive-chart'), [{
				...
				label: label, 
				color: COLOR_BLUE,
				lines: { show: true, fill:true, lineWidth: 2.5 },
			...
			yaxis: {  tickColor: COLOR_GREY_LIGHTER, tickSize: 300 },
			grid: { 
				hoverable: true, 
				clickable: true,
...
var Chart = function () {
    ...
    return {
        ...
        init: function () {
            ...
            handleStackedChart();
            handleTrackingChart();
            handleBarChart(d2,label);
            handleInteractivePieChart();
            handleDonutChart();
            handleInteractiveChart(d1,label);
